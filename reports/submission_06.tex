...
\end{lstlisting} 
Summary: Current user has their ID pushed on to the item's dislikeCounter with id :itemid.\\
Does: Remove user's id from the likeCounter\\
Replaces: \\
Updates: the likeCounter array for the specified item in the database\\
Who is authorized: Every logged in user is authorized to like an item\\
...
\end{lstlisting} 
Summary: Sends a message which gets logged by both the sender and receiver\\
Does: \\
Replaces: \\
Updates: \\
Who is authorized: \\
...
GET /user/:userid/pm
\end{lstlisting} 
Summary: Returns the productManager of a user with :userid with all their items for sale.\\
Does: \\
Replaces: \\
Updates: \\
Who is authorized: \\
...
\end{lstlisting} 
Summary: Returns a list of all the messages for a particular user\\
Does: \\
Replaces: \\
Updates: \\
Who is authorized: \\
...
Richard Cui contributed to the Chat component.
\end{homeworkSection}

\begin{homeworkSection}{Roman Ganchin}
Roman Ganchin contributed to the Trending, Category, Item, and Report components.
\end{homeworkSection}

\begin{homeworkSection}{Gahyun (Susie) Kim}
Gahyun (Susie) Kim implemented the error banner for server issues and fixed the button on uploaditem to allow the user to select a file.
\end{homeworkSection}
...
The user sees all items in trending and not only the items in his own feed. \newline
When clicking an item from trending the functionality works as expected but the stays on initial clicked item. \newline
\end{homeworkProblem}
\newpage
%----------------------------------------------------------------------------------------
