...
POST /upload [item]
\end{lstlisting} 
Where [item] is a JSON object describing a new item for sale\\
\begin{lstlisting}
PUT /feeds/:userid/item/:itemid/likeCounter
\end{lstlisting} 
Current user has their ID pushed on to the item's likeCounter with id :itemid\\
\begin{lstlisting}
PUT /feeds/:userid/item/:itemid/dislikeCounter
\end{lstlisting} 
Current user has their ID pushed on to the item's dislikeCounter with id :itemid\\
\begin{lstlisting}
DELETE /pm/:userid/item/:itemid
\end{lstlisting} 
Removes item from vendors ProductManager and from the feeds of anyone that has this item\\
\end{homeworkProblem}
